/*
    ...
    Uses the AssetTracker.h library instead of Adafruit_GPS.h
...
void setup(){
    ...
    at.begin();                                                                                             // Setup AssetTracker
    at.gpsOff();                                                                                            // Disable GPS module (If on)
...
void getSensorData(){
    ...
    if(!at.gpsFix() && WAIT_FOR_SAT_FIX){
        ...
        while(!at.gpsFix() && WAIT_FOR_SAT_FIX){
            long now = millis();
            ...
            if(((now - last_gps_fix_check_at) > DELAY_WHILE_WAIT_FOR_SAT_FIX) || last_gps_fix_check_at <= 0){
                ...
                // If number of checks exceed the max number of checks allowed, exit
    ...
    // Get GPS location
    print("Reading location", false);
    
    String gpsData;
    if(at.gpsFix()){
        gpsData = at.readLatLon();
        print(": " + String(at.readLatLon()), false);
        print(" -> Done", true);
    }
    else{
    ...
    String data_str = "{\"do\":" + String(do_float) + ",\"ec\":" + String(ec_float) + ",\"ph\":" + String(ph_float) + ",\"rtd\":" + String(rtd_float) + ",\"location\":\"" + String(gpsData) + "\"" +  + ",\"number_of_sat_fix_checks\":\"" + number_of_sat_fix_checks + "\"" + ",\"batt-volt\":" + fuel.getVCell() + ",\"batt-level\":" + fuel.getSoC() + ",\"utc-timestamp\":\"" + getTime("utc") + "\"}";
